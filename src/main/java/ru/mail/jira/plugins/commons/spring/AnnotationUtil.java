...
public class AnnotationUtil {
  ...
  public static boolean isAnnotated(Class<? extends Annotation> annotationClass, Method method) {
    return method != null
        && (isAnnotationPresent(annotationClass, method)
            || isAnnotationPresent(annotationClass, method.getDeclaringClass()));
  }
      ...
      if (c.isInterface()) {
        ...
        if (isAnnotationPresent(annotationClass, c)) {
          throw new InvalidAnnotationException(
              "Transactional is not supported for concrete classes");
        }
        for (Method method : c.getMethods()) {
          if (isAnnotated(annotationClass, method)) {
            throw new InvalidAnnotationException(
  ...
  private static boolean isAnnotationPresent(
      Class<? extends Annotation> annotationClass, AnnotatedElement e) {
