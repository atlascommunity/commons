...
public class ContractUtils {
  ...
  private static final Pattern SNILS_WITH_CHECKSUM_PATTERN =
      Pattern.compile("(\\d{3}-\\d{3}-\\d{3}) (\\d{2})");
  private static final Pattern SNILS_WITHOUT_CHECKSUM_PATTERN =
      Pattern.compile("\\d{3}-\\d{3}-\\d{3}");
  private static final Pattern EXTRA_SNILS_VALIDATE_PATTERN =
  ...
  public static boolean isValidSnils(String snils) {
    ...
    if (snilsWithCsMatcher.matches()) {
      ...
      if (EXTRA_SNILS_VALIDATE_PATTERN.matcher(joinedPartBeforeSpace).find()) return false;

      int sum = 0;
      for (int i = 0, j = 9; i < 9; i++, j--)
        sum += Character.getNumericValue(joinedPartBeforeSpace.charAt(i)) * j;

      if (sum < 100) return checksum == sum;
      else if (sum == 100 || sum == 101) return checksum == 0;
      else return (checksum == sum % 101) || (checksum == 0 && sum % 101 == 100);
    }

    if (SNILS_WITHOUT_CHECKSUM_PATTERN.matcher(snils).matches()) {
      String joinedPartBeforeSpace = snils.replaceAll("-", "");
      return !EXTRA_SNILS_VALIDATE_PATTERN.matcher(joinedPartBeforeSpace).find()
          && Integer.valueOf(joinedPartBeforeSpace) <= 1001998;
    }
